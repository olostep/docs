...
Via le point de terminaison Olostep `/v1/files`, tu peux télécharger des fichiers JSON qui peuvent être utilisés comme contexte dans tes requêtes API. Cela te permet de fournir des données structurées pour améliorer tes extractions, réponses et autres opérations.

- Télécharge des fichiers JSON jusqu'à 200 Mo
- Les fichiers sont automatiquement validés pour un format JSON correct
- Utilise les fichiers comme contexte dans les extractions, réponses et autres points de terminaison
- Les fichiers expirent après 30 jours
- Processus de téléchargement sécurisé via URL pré-signée

Pour les détails de l'API, consulte la [Référence de l'API du point de terminaison Fichiers](/api-reference/files/create).
...
```bash cURL
# macOS : curl intégré est suffisant
```
...
2. **Compléter le téléchargement** : Télécharge ton fichier sur l'URL pré-signée, puis appelle le point de terminaison de complétion pour valider et finaliser

### Étape 1 : Créer une URL de téléchargement

Tout d'abord, crée une URL de téléchargement en fournissant le nom du fichier et le but optionnel. Le paramètre `purpose` ne prend en charge que deux valeurs : `"context"` (par défaut) ou `"batch"`.
...
Télécharge ton fichier JSON sur l'URL pré-signée, puis appelle le point de terminaison de complétion pour valider et finaliser le téléchargement.
...
# Étape 2a : Télécharger le fichier sur l'URL pré-signée
...
Le point de terminaison de complétion valide le fichier JSON et renvoie les métadonnées du fichier :
...
Obtiens une URL pré-signée pour télécharger le contenu JSON d'un fichier complété. Spécifie éventuellement le temps d'expiration pour l'URL de téléchargement en utilisant le paramètre de requête `expires_in` (par défaut à 600 secondes / 10 minutes).
...
Liste tous les fichiers complétés pour ton équipe. Filtre éventuellement par but (valeurs prises en charge : `"context"` ou `"batch"`).
...
# Lister les fichiers filtrés par but
response = requests.get(
...
## Exemple de téléchargement complet (but context)
...
Voici un exemple qui télécharge un fichier JSON avec `purpose="batch"` contenant des données batch valides pouvant être utilisées avec le point de terminaison `/v1/batches` :
...
# Étape 2 : Préparer les données JSON batch (format valide pour le point de terminaison /v1/batches)
batch_data = {
    "items": [
...
console.log(`Taille du fichier : ${fileInfo.bytes} octets`)
console.log(`But : ${fileInfo.purpose}`)
```
...
Le fichier batch téléchargé contient une structure JSON valide qui correspond au format du point de terminaison `/v1/batches` :
...
- **Paramètre purpose** : Prend uniquement en charge les valeurs `"context"` ou `"batch"` (par défaut à `"context"`)

## Tarification

Les téléchargements de fichiers sont gratuits. Les fichiers sont stockés de manière sécurisée et expirent automatiquement après 30 jours.