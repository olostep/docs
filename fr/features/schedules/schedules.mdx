...
title: "Horaires"
description: "Planifiez les appels d’API pour s’exécuter automatiquement à des heures spécifiées"
icon: "clock"
---

Grâce à l’endpoint Olostep `/v1/schedules`, tu peux planifier des appels d’API pour qu’ils s’exécutent automatiquement à des heures spécifiées. Planifie des exécutions uniques ou des tâches récurrentes en utilisant des expressions cron ou un langage naturel.
...
- Transmets n’importe quelle charge utile que tu souhaites - la charge est envoyée exactement comme tu la spécifies
- Gère automatiquement le cycle de vie des horaires

Pour les détails de l’API, consulte la [Référence de l’API Endpoint d’Horaires](/api-reference/schedules/create).
...
Crée un horaire pour exécuter automatiquement des appels d’API. Tu peux créer des horaires uniques ou récurrents en utilisant des expressions cron. L’`endpoint` peut être n’importe quelle URL (pas limité aux endpoints Olostep), et la `charge utile` peut contenir toutes les données que tu souhaites envoyer.

### Horaire unique

Planifie un appel d’API pour s’exécuter une fois à une date et heure spécifiques.
...
Supprime un horaire par son ID. Cela arrêtera toute exécution future.
...
- `v1/scrapes` - Planifie des tâches de web scraping
...
- `0 10 * * ? *` - Tous les jours à 10:00
- `0 9 ? * MON *` - Tous les lundis à 9:00
...
- Les horaires récurrents continuent jusqu’à suppression manuelle
...
- Les horaires exécutent l’appel d’API avec la charge utile fournie exactement comme spécifié - tu peux transmettre n’importe quelle structure JSON dont tu as besoin
- Pour les requêtes POST, les endpoints Olostep en format court (`v1/scrapes`, `v1/batches`, `v1/crawls`, `v1/maps`, `v1/answers`) sont automatiquement préfixés avec `https://api.olostep.com/`
- Pour les autres endpoints, fournis l’URL complète
- La `charge utile` peut contenir n’importe quelle structure de données - elle est envoyée telle quelle à ton endpoint cible
- Supprimer un horaire déjà supprimé renverra une erreur 400

## Tarification

Les horaires eux-mêmes sont gratuits. Tu ne paies que pour les appels d’API qui sont exécutés lorsque l’horaire s’exécute. Par exemple, si tu planifies une extraction, tu seras facturé 1 crédit par exécution (ou plus si tu utilises des parseurs ou une extraction LLM).