---
icon: "map"
sidebarTitle: "Résultats Google Maps"
title: "Obtenir des Résultats Google Maps"
description: "API SERP pour extraire des données de localisation Google Maps en utilisant Olostep et un extrait de code Python."
---

## Vue d'ensemble

L'API d'Olostep te permet d'extraire des données structurées des localisations Google Maps en utilisant des parseurs. Ces parseurs transforment le HTML brut des pages Google Maps en données JSON propres et structurées, prêtes pour l'analyse ou l'intégration dans tes applications.

## Exemple d'Intégration

Pour obtenir le contenu JSON analysé des résultats Google Maps, tu dois inclure `json` dans le paramètre formats et spécifier le nom du parseur `@olostep/google-maps` dans l'objet parser. Voici comment récupérer les données de localisation Google Maps dans un format structuré :
...
## Format de Réponse
Lorsque tu fais une requête à l'API Olostep avec le parseur Google Maps, tu recevras une réponse JSON comme l'exemple ci-dessous :
...
- `Détails de l'entreprise` (nom, adresse, informations de contact)
- `Informations sur les évaluations et les avis`
- `Horaires d'ouverture`
- `Avis des utilisateurs avec texte et images`
- `Lieux similaires à proximité`
- `Tags populaires et commodités`

## Réponse Structurée : json_content

Le champ json_content contient des informations détaillées sur la localisation. Voici un exemple de données que tu peux attendre :
...
Olostep fournit un fichier JSON hébergé avec les données de localisation structurées. Tu peux accéder au fichier JSON en utilisant le champ json_hosted_url dans la réponse :

- JSON structuré : [Voir l'exemple de JSON](https://olostep-storage.s3.us-east-1.amazonaws.com/json_y6jwelcpdl.json)

Si tu souhaites également obtenir le contenu HTML et Markdown des données de localisation, tu peux inclure ces formats dans le paramètre formats et Olostep les renverra dans la réponse et fournira des URLs hébergées pour chaque format.

## Cas d'Utilisation
Le parseur Google Maps est particulièrement utile pour :

Intelligence économique : Collecter des données sur les localisations, évaluations et retours clients des concurrents
Études de marché : Analyser les avis et sentiments des clients sur les entreprises dans des zones spécifiques
Applications basées sur la localisation : Intégrer des informations détaillées sur les entreprises et des avis clients
Analyse immobilière : Rassembler des données sur les commodités à proximité et les entreprises locales
SEO local : Surveiller la présence des entreprises et les avis clients sur Google Maps

## Notes Importantes

Paramètre de langue : Le paramètre ?hl=en définit la langue sur l'anglais. Ajuste-le selon les besoins pour d'autres langues.

## Conclusion
Utiliser le parseur Google Maps avec l'API d'Olostep te permet d'extraire des données de localisation structurées qui peuvent être facilement intégrées dans tes applications ou utilisées pour l'analyse. Le parseur gère la tâche complexe d'extraction d'informations des pages Google Maps, te fournissant des données propres et structurées.
Pour obtenir des informations sur les parseurs disponibles ou pour demander un parseur personnalisé pour ton cas d'utilisation spécifique, contacte-nous à `info@olostep.com`