---
icon: "google"
sidebarTitle: "Résultats de Recherche Google"
title: "Obtenir les Résultats de Recherche Google (python)"
description: "API SERP pour extraire les résultats de recherche Google en utilisant Olostep et un extrait de code Python."
---

## Vue d'ensemble

L'API d'Olostep te permet d'extraire des données structurées des résultats des moteurs de recherche en utilisant des [parseurs](../../features/structured-content/parsers). Ces parseurs transforment le HTML brut des résultats de recherche en données JSON propres et structurées, prêtes pour l'analyse ou l'intégration dans tes applications.

## Exemple d'Intégration

Pour obtenir le contenu JSON analysé des résultats de recherche, tu dois inclure `json` dans le paramètre `formats` et spécifier le nom du parseur `@olostep/google-search` dans l'objet `parser`.

Voici comment récupérer les résultats de recherche Google dans un format structuré :
...
Lorsque tu fais une requête à l'API Olostep avec le parseur Google Search, tu recevras une réponse JSON comme l'exemple ci-dessous :
...
- JSON structuré : [Voir exemple JSON](https://olostep-storage.s3.us-east-1.amazonaws.com/json_vxc86vq2pf.json)
...
**Paramètres de Recherche** : Les paramètres `gl=us` et `hl=en` définissent la géolocalisation aux États-Unis et la langue en anglais. Ajuste-les selon tes besoins.

## Conclusion

Passer des parseurs personnalisés à l'API te permet d'obtenir uniquement les données structurées que tu souhaites. Cela facilite l'intégration des résultats de recherche dans tes applications, l'analyse des données ou la création de fonctionnalités liées à la recherche.

Pour obtenir des informations sur les parseurs disponibles ou pour demander un parseur personnalisé pour ton cas d'utilisation spécifique, merci de nous contacter à [info@olostep.com](mailto:info@olostep.com).