...
Files with `purpose="batch"` are the recommended way to provide URL lists to [long-running batches](/features/batches/batches#long-running-batches) — large workloads (10,000+ URLs) that run asynchronously for up to 12 hours.
...
- `items`: Array of objects with `custom_id` and `url` fields (a plain array is also accepted)
- `parser`: Optional parser configuration
- `country`: Optional country code

Once uploaded and completed, pass the `file_id` to `/v1/batches` with `async: true` and a `completion_window` to start a [long-running batch](/features/batches/batches#long-running-batches).
