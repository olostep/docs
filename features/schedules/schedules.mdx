...
- Schedule any HTTP endpoint (GET or POST) - works with any URL, not just Olostep endpoints
...
const executeAt = new Date(Date.now() + 60 * 60 * 1000).toISOString() // 1 hour from now
...
curl -s -X POST "https://api.olostep.com/v1/schedules" \
  ...
  -d '{
    ...
    "executeAt": "2025-01-15T10:00:00Z",
    "expressionTimezone": "UTC"
  }'
```
...
Create a recurring schedule using a cron expression. Cron expressions use AWS EventBridge format (6 fields): minute hour day month day-of-week year.
...
const res = await fetch(`${API_URL}/schedules`, {
  ...
  body: JSON.stringify({
    ...
    cronExpression: '0 10 * * ? *', // Every day at 10am UTC
    expressionTimezone: 'UTC'
  })
})
console.log(await res.json())
...
For one-time schedules, the response includes `executeAt` instead of `cronExpression`:
...
```json
{
  ...
  "method": "POST",
  "endpoint": "v1/scrapes",
  "executeAt": "2025-01-15T10:00:00.000Z",
  "expressionTimezone": "UTC",
...
Retrieve all schedules for your team.
...
You can schedule **any HTTP endpoint** - it's not limited to Olostep endpoints. The `endpoint` field accepts any URL, and you can pass any `payload` you want.

### Olostep endpoints (short form)

For convenience, you can use short forms for common Olostep endpoints. The system will automatically prepend `https://api.olostep.com/` for these:
...
You can also schedule calls to any external API or webhook. Just provide the full URL:
...
# Schedule a call to any external API
payload = {
    "method": "POST",
    ...
    "executeAt": (datetime.now() + timedelta(hours=1)).isoformat(),
    "expressionTimezone": "UTC"
}
...
Cron expressions use AWS EventBridge format with 6 fields:
...
- The `executeAt` datetime must be in the future
...
- The `endpoint` can be any URL - it doesn't have to be an Olostep endpoint
