---
icon: "globe"
sidebarTitle: "GEO - AI検索ツール"
title: "人気のAI検索ツールからデータを取得する"
description: "バッチリクエストを使用してAI搭載の検索エンジンから構造化データを抽出する"
---

## 概要

このガイドでは、OlostepのBatchesエンドポイントを使用して、Google AI Mode、Gemini、Microsoft Copilot、ChatGPT、Perplexity、Google AI Overviewなどの人気AI検索ツールからデータを取得する方法を示します。

各プラットフォームに特化したパーサーを活用することで、大規模な構造化検索結果を抽出でき、以下の用途に最適です：
- 競合情報の収集
- 複数プラットフォームの検索結果の比較
- AI検索エンジンの監視
...
検索したいクエリの配列を作成します。このデモでは、異なる都市でのニュースをクエリします：
...
**注意:** 送信後、結果を取得するために[完了をポーリング](/use-cases/batch#step-2-monitor-batch-status)してください。

## レスポンスフォーマット

バッチリクエストを送信し、完了をポーリングした後、以下のフォーマットでレスポンスを受け取ります：
...
以下の表は、国別検索のサポートと各パーサーによるクレジット消費を示しています。

| 機能             | Google AI Mode | ChatGPT | Perplexity | Google AI Overview | Gemini | Microsoft Copilot | Grok |
| ------------------- | :------------: | :-----: | :--------: | :----------------: | :----: | :---------------: | :--: |
| **サポートされる国** |       ✅       |    ✅     |     ✅     |         ✅         |   ✅   |        ✅         |  ✅  |
| **バッチサイズ制限** |       2500      |    2500     |     2500     |         2500         |   2500   |        1000         |  1000  
| **クレジット**         |       3        |    5    |     3      |         3          |   3    |         3         |  3   |

### サポートされる国の確認

各パーサーのサポートされる国のリストを取得するには、以下のエンドポイントを使用します：
...
何か不足していますか？サポートやカスタム実装の支援については、`info@olostep.com`までお問い合わせください。