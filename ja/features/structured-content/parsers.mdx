---
sidebarTitle: "パーサーの使用"
title: "パーサーの使用"
---

パーサーは、非構造化データをバックエンドと互換性のある構造化データに変換する方法です。パーサーをOlostep API（クロール、スクレイプ、バッチ）と組み合わせることで、任意のウェブサイトをAPIコールに変換し、必要なJSONを返すことができます。

パーサーは、同じウェブサイトから定期的に大量のデータが必要な場合に理想的です。このアプローチは、[LLM抽出](/features/structured-content/llm-extraction)と比較して、コスト効率が高く、迅速で、必要なJSONのみを返します。

私たちは人気のあるウェブサイトやユースケースに対して事前に構築されたパーサーを提供しています。また、[ダッシュボード](https://www.olostep.com/dashboard/parsers)を使用してLLMで数分で独自のパーサーを作成することもできますし、私たちのチームに依頼することもできます。

### 事前構築されたパーサー

人気のあるウェブサイト向けにいくつかの事前構築されたパーサーを提供しています：
- Google検索: `@olostep/google-search`
- Amazon商品: `@olostep/amazon-it-product`
- メール抽出: `@olostep/extract-emails`
- カレンダー抽出: `@olostep/extract-calendars`
- ソーシャル抽出: `@olostep/extract-socials`
- TikTokデータ抽出: パーサーIDを取得するにはお問い合わせください
...
パーサーフォーマットでOlostep APIにリクエストを送ると、以下の例のようなJSONレスポンスを受け取ります：
...
`json_content`は、構造化されたJSON結果を含むレスポンスの主要部分です。レスポンスから直接JSONコンテンツにアクセスするか、レスポンスで提供されるホストされたURLを使用してアクセスできます。
...
Olostepは、構造化された結果を含むホストされたJSONファイルも提供しています。レスポンスの`json_hosted_url`フィールドを使用してJSONファイルにアクセスできます：

- 構造化されたJSON: [例のJSONを表示](https://olostep-storage.s3.us-east-1.amazonaws.com/json_vxc86vq2pf.json)

検索結果のHTMLとMarkdownコンテンツも取得したい場合は、`formats`パラメータにこれらの形式を含めると、Olostepがレスポンスでそれらを返し、各形式のホストされたURLを提供します。
- [Markdown形式](https://olostep-storage.s3.us-east-1.amazonaws.com/markDown_vxc86vq2pf.txt)
- [HTML](https://olostep-storage.s3.us-east-1.amazonaws.com/text_vxc86vq2pf.txt)

### カスタムパーサーが必要ですか？

特定のウェブサイト用のパーサーや事前構築されたパーサーのIDが必要な場合：
