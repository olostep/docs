---
sidebarTitle: "Uso de Parsers"
title: "Uso de Parsers"
---

Los parsers son una forma de convertir datos no estructurados en datos estructurados que son compatibles con tu backend. Combinar parsers con la API de Olostep (rastreo, scraping, lotes) permite convertir cualquier sitio web en una llamada de API para devolver el JSON que necesitas.

Los parsers son ideales cuando necesitas datos a gran escala de manera recurrente de los mismos sitios web. Este enfoque es significativamente más rentable y rápido en comparación con [extracción LLM](/features/structured-content/llm-extraction) y devuelve solo el JSON requerido.

Ofrecemos parsers preconstruidos para sitios web y casos de uso populares. También puedes crear tus propios parsers en pocos minutos con LLMs usando el [dashboard](https://www.olostep.com/dashboard/parsers) o pedirle a nuestro equipo que lo haga por ti.

### Parsers Preconstruidos

Ofrecemos varios parsers preconstruidos para sitios web populares:
- Búsqueda de Google: `@olostep/google-search`
- Producto de Amazon: `@olostep/amazon-it-product`
- Extraer Email: `@olostep/extract-emails`
- Extraer Calendarios: `@olostep/extract-calendars`
- Extraer Redes Sociales: `@olostep/extract-socials`
- Extracción de datos de TikTok: ponte en contacto con nosotros para obtener el ID del parser
- Noticias de Google: ponte en contacto con nosotros para obtener el ID del parser
- Google Maps: ponte en contacto con nosotros para obtener el ID del parser
...
Cuando haces una solicitud a la API de Olostep con el formato de parser, recibirás una respuesta JSON como el ejemplo a continuación:
...
- **json_content** con resultados JSON estructurados que incluyen:
...
`json_content` es la parte principal de la respuesta con los resultados JSON estructurados. Puedes acceder al contenido JSON directamente desde la respuesta o usar la URL alojada proporcionada en la respuesta.
...
Olostep también proporciona un archivo JSON alojado con los resultados estructurados. Puedes acceder al archivo JSON usando el campo `json_hosted_url` en la respuesta:

- JSON estructurado: [Ver ejemplo JSON](https://olostep-storage.s3.us-east-1.amazonaws.com/json_vxc86vq2pf.json)

Si también deseas obtener el contenido HTML y Markdown de los resultados de búsqueda, puedes incluir estos formatos en el parámetro `formats` y Olostep los devolverá en la respuesta y proporcionará URLs alojadas para cada formato.
...
### ¿Necesitas un Parser Personalizado?

Si necesitas un parser para un sitio web específico o el ID de un parser preconstruido:

- Contáctanos en info@olostep.com