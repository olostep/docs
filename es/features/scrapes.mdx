...
A través del endpoint de Olostep `/v1/scrapes` puedes extraer Markdown, HTML, texto, capturas de pantalla o JSON estructurado amigable para LLM desde cualquier URL en tiempo real.
...
Para detalles del API, consulta la [Referencia del API del Endpoint Scrape](/api-reference/scrapes/create).

## Scraping de una URL

Usa el endpoint `/v1/scrapes` para hacer scraping de una sola URL y elegir formatos de salida.
...
Puedes usar el endpoint para hacer scraping de una sola URL y elegir formatos de salida. Los parámetros obligatorios son `url_to_scrape` y `formats`.
...
Olostep tiene algunos parsers preconstruidos para [sitios web populares](https://www.olostep.com/store) pero también puedes crear tus propios parsers a través del panel de control o pedirle a nuestro equipo que lo haga por ti.
...
Realiza acciones antes de hacer scraping para interactuar con sitios dinámicos. Acciones soportadas:
...
- **Análisis de Reseñas**: Recopila comentarios y sentimientos de consumidores para productos específicos
...
- **Archivos de Datos Históricos**: Preserva el contenido de sitios web en momentos específicos
...
- **Monitoreo del Estado del Servicio**: Verifica páginas de estado del servicio para plataformas o herramientas específicas
...
Scrape cuesta 1 crédito por defecto. Si también pasas [parsers](/features/structured-content/parsers), los costos varían por parser (1-5 créditos). Si usas [LLM extract](/features/structured-content/llm-extraction), cuesta 20 créditos.