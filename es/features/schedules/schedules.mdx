---
sidebarTitle: "Programar"
title: "Programar"
description: "Programa llamadas a la API para que se ejecuten automáticamente en momentos específicos"
icon: "clock"
---

A través del endpoint de Olostep `/v1/schedules`, puedes programar llamadas a la API para que se ejecuten automáticamente en momentos específicos. Programa ejecuciones únicas o tareas recurrentes usando expresiones cron o lenguaje natural.

- Programa ejecuciones únicas en una fecha y hora específicas
- Crea programaciones recurrentes usando expresiones cron
...
- Envía cualquier payload que desees - el payload se envía exactamente como lo especifiques
- Gestiona automáticamente el ciclo de vida de la programación

Para detalles de la API, consulta la [Referencia de la API del Endpoint de Programación](/api-reference/schedules/create).
...
Crea una programación para ejecutar llamadas a la API automáticamente. Puedes crear programaciones únicas o recurrentes usando expresiones cron. El `endpoint` puede ser cualquier URL (no limitado a endpoints de Olostep), y el `payload` puede contener cualquier dato que desees enviar.
...
### Programación recurrente con expresión cron

Crea una programación recurrente usando una expresión cron. Las expresiones cron usan un formato de 6 campos: minuto hora día mes día-de-la-semana año.
...
Cuando creas una programación, recibirás un objeto de programación con las siguientes propiedades:
...
Para programaciones únicas, la respuesta incluye `execute_at` en lugar de `cron_expression`:
...
Recupera todas las programaciones para tu equipo. Por defecto, las programaciones eliminadas están filtradas. Usa el parámetro de consulta `include_deleted` para incluirlas.
...
print(f"Total de programaciones: {result['count']}")
for schedule in result['schedules']:
    print(json.dumps(schedule, indent=2))

# Para incluir programaciones eliminadas:
...
console.log(`Total de programaciones: ${result.count}`)
...
Recupera una sola programación por su ID.
...
Elimina una programación por su ID. Esto detendrá cualquier ejecución futura.
...
Para solicitudes POST, puedes usar formas cortas para los endpoints de Olostep. El sistema automáticamente añadirá `https://api.olostep.com/` a estos:
...
Puedes usar lenguaje natural para describir programaciones. El sistema las convertirá automáticamente en expresiones cron:

- "cada 3 minutos" → `0/3 * * * ? *`
- "todos los días a las 10am" → `0 10 * * ? *`
- "todos los lunes a las 9am" → `0 9 ? * MON *`
- "cada hora" → `0 * * * ? *`
- "cada semana el lunes" → `0 0 ? * MON *`

## Notas importantes

- Las programaciones únicas se eliminan automáticamente después de la ejecución
- Las programaciones recurrentes continúan hasta que se eliminen manualmente
- La zona horaria debe ser un identificador de zona horaria IANA válido (por ejemplo, "UTC", "America/New_York", "Europe/London")
- La fecha y hora de `execute_at` debe estar en el futuro
- La conversión de lenguaje natural puede requerir reintentos; el sistema intentará hasta 3 veces
- Al usar texto en lenguaje natural (parámetro `text`), la zona horaria predeterminada es "UTC"
- Las programaciones ejecutan la llamada a la API con el payload proporcionado exactamente como se especifica - puedes pasar cualquier estructura JSON que necesites
...
- Eliminar una programación ya eliminada devolverá un error 400

## Precios

Las programaciones en sí son gratuitas. Solo pagas por las llamadas a la API que se ejecutan cuando la programación se ejecuta. Por ejemplo, si programas un scrape, se te cobrará 1 crédito por ejecución (o más si usas parsers o extracción LLM).