...
title: "Horarios"
description: "Programa llamadas a la API para ejecutarse automáticamente en momentos específicos"
icon: "clock"
---
...
- Para solicitudes POST, usa endpoints de Olostep en formato corto (prefijados automáticamente) o URLs completas
...
Crea un horario para ejecutar llamadas a la API automáticamente. Puedes crear horarios únicos o recurrentes usando expresiones cron. El `endpoint` puede ser cualquier URL (no limitado a endpoints de Olostep), y el `payload` puede contener cualquier dato que desees enviar.

### Horario único

Programa una llamada a la API para ejecutarse una vez en una fecha y hora específicas.
...
# Programa un scraping para ejecutarse en 1 hora
...
const execute_at = new Date(Date.now() + 60 * 60 * 1000).toISOString() // 1 hora a partir de ahora
...
# Programa un scraping para ejecutarse todos los días a las 10am UTC
payload = {
    "method": "POST",
    "endpoint": "v1/scrapes",
    "payload": {
...
Recupera un único horario por su ID.
...
### Endpoints de Olostep (formato corto)
...
- Los horarios recurrentes continúan hasta que se eliminan manualmente
...
- Para solicitudes POST, los endpoints de Olostep en formato corto (`v1/scrapes`, `v1/batches`, `v1/crawls`, `v1/maps`, `v1/answers`) se prefijan automáticamente con `https://api.olostep.com/`
...
Los horarios en sí son gratuitos. Solo pagas por las llamadas a la API que se ejecutan cuando el horario se activa. Por ejemplo, si programas un scraping, se te cobrará 1 crédito por ejecución (o más si usas parsers o extracción LLM).