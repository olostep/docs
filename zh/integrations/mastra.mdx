...
icon: "robot"
title: "Mastra 集成"
description: "使用 Mastra.ai 的代理框架构建具有网页搜索、抓取和爬虫功能的 AI 代理"
---

Olostep Mastra 集成为 Mastra.ai 代理带来了强大的网页数据提取能力。Olostep 是一个网页搜索、抓取和爬虫 API——用于搜索、提取和结构化网页数据的 API。构建智能 AI 代理，可以自主搜索、抓取、分析和结构化任何网站的数据。
...
<CardGroup cols={2}>
  ...
  <Card title="批量抓取 URL" icon="layer-group">
    并行处理多达 100,000 个 URL，适合大规模数据提取
  </Card>
  
  <Card title="创建爬虫" icon="spider-web">
    自主发现并抓取整个网站，通过链接进行导航
  </Card>
  
  <Card title="创建地图" icon="map">
    提取网站的所有 URL 用于网站结构分析和内容发现
  </Card>
</CardGroup>
...
Olostep 提供了针对热门网站的预构建解析器。使用 `parser` 参数：
...
<AccordionGroup>
  ...
  <Accordion title="使用专用解析器">
    对于热门网站（如亚马逊、LinkedIn、Google），使用预构建解析器：
    - 自动获取结构化数据
    - 提取更可靠
    - 无需自定义解析
    - 由 Olostep 维护
...
Olostep 根据 API 使用收费，与 Mastra 独立：
...
<CardGroup cols={2}>
  ...
  <Card title="爬虫 API" icon="spider-web" href="/features/crawls/crawls">
    了解爬虫端点
  </Card>
...
<Card title="安装包" icon="download" href="https://www.npmjs.com/package/@olostep/mastra-tools">
  从 npm 安装 @olostep/mastra-tools
</Card>

使用 Olostep 和 Mastra 构建能够搜索、提取和结构化网页数据的智能 AI 代理！