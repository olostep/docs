...
sidebarTitle: "计划"
title: "计划"
description: "计划 API 调用在指定时间自动运行"
icon: "clock"
---
...
- 对于 POST 请求，使用简短形式的 Olostep 端点（自动加前缀）或完整 URL
- 传递你想要的任何负载 - 负载会按你指定的方式发送
- 自动管理计划生命周期

有关 API 详细信息，请参见 [计划端点 API 参考](/api-reference/schedules/create)。
...
使用 cron 表达式创建一个定期计划。Cron 表达式使用 6 个字段格式：分钟 小时 日 月 星期几 年。
...
const res = await fetch(`${API_URL}/schedules`, {
  ...
  body: JSON.stringify({
    ...
    cron_expression: '0 10 * * ? *', // 每天上午 10 点 UTC
    expression_timezone: 'UTC'
  })
})
console.log(await res.json())
...
检索你团队的所有计划。默认情况下，已删除的计划会被过滤掉。使用 `include_deleted` 查询参数来包含它们。
...
对于 POST 请求，你可以使用 Olostep 端点的简短形式。系统会自动为这些加上 `https://api.olostep.com/` 前缀：
...
你可以使用自然语言来描述计划。系统会自动将它们转换为 cron 表达式：
...
- 自然语言转换可能需要重试；系统最多会尝试 3 次
- 使用自然语言文本（`text` 参数）时，时区默认为 "UTC"
- 计划会使用提供的负载精确执行 API 调用 - 你可以传递任何需要的 JSON 结构
- 对于 POST 请求，简短形式的 Olostep 端点（`v1/scrapes`, `v1/batches`, `v1/crawls`, `v1/maps`, `v1/answers`）会自动加上 `https://api.olostep.com/` 前缀
- 对于其他端点，请提供完整的 URL
...
计划本身是免费的。你只需为计划运行时执行的 API 调用付费。例如，如果你安排一次抓取，你将为每次执行支付 1 个信用（如果使用解析器或 LLM 提取则更多）。