...
检索你的团队的所有计划。默认情况下，已删除的计划会被过滤掉。使用 `include_deleted` 查询参数可以包含它们。
...
`payload` 字段接受任何 JSON 对象 - 你可以根据目标端点的需要来构建它。
...
计划本身是免费的。你只需为计划运行时执行的 API 调用付费。例如，如果你安排一次抓取，每次执行将收取 1 个信用（如果使用解析器或 LLM 提取则更多）。