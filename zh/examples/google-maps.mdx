---
icon: "map"
sidebarTitle: "Google Maps 结果"
title: "获取 Google Maps 结果"
description: "使用 Olostep 和 Python 代码片段抓取 Google Maps 位置数据的 SERP API。"
---

## 概述

Olostep 的 API 允许你通过使用解析器从 Google Maps 位置提取结构化数据。这些解析器将 Google Maps 页面原始 HTML 转换为干净、结构化的 JSON 数据，方便分析或集成到你的应用程序中。

## 集成示例

要从 Google Maps 结果中获取解析后的 JSON 内容，你需要在 formats 参数中包含 `json`，并在解析器对象中指定解析器的名称 `@olostep/google-maps`。以下是如何以结构化格式检索 Google Maps 位置数据：
...
当你使用 Google Maps 解析器向 Olostep API 发出请求时，你将收到如下示例的 JSON 响应：
...
json_content 字段包含关于位置的详细信息。以下是你可以预期的数据示例：
...
Olostep 提供了一个托管的 JSON 文件，包含结构化的位置信息。你可以使用响应中的 json_hosted_url 字段访问 JSON 文件：

- 结构化 JSON：[查看示例 JSON](https://olostep-storage.s3.us-east-1.amazonaws.com/json_y6jwelcpdl.json)

如果你还想获取位置数据的 HTML 和 Markdown 内容，可以在 formats 参数中包含这些格式，Olostep 将在响应中返回它们，并提供每种格式的托管 URL。

## 使用案例

Google Maps 解析器特别适用于：

商业智能：收集关于竞争对手位置、评分和客户反馈的数据
市场研究：分析特定区域内企业的客户评论和情感
基于位置的应用程序：集成详细的商业信息和客户评论
房地产分析：收集有关附近设施和本地企业的数据
本地 SEO：监控 Google Maps 上的商业存在和客户评论

## 重要提示

语言参数：?hl=en 参数将语言设置为英语。根据需要调整为其他语言。

## 结论

使用 Olostep 的 API 和 Google Maps 解析器可以提取结构化的位置信息，方便集成到你的应用程序中或用于分析。解析器处理从 Google Maps 页面提取信息的复杂任务，为你提供干净、结构化的数据。
如需获取可用解析器的信息或请求自定义解析器以满足你的特定使用案例，请通过 `info@olostep.com` 联系我们。