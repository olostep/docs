---
icon: "globe"
sidebarTitle: "GEO - AI 搜索工具"
title: "从流行的 AI 搜索工具获取数据"
description: "使用批量请求从 AI 驱动的搜索引擎提取结构化数据"
---

## 概述

本指南演示如何使用 Olostep 的 Batches 端点从流行的 AI 搜索工具（包括 Google AI Mode、Gemini、Microsoft Copilot、ChatGPT、Perplexity 和 Google AI Overview）获取数据。

通过利用每个平台的专用解析器，你可以大规模提取结构化的搜索结果，非常适合用于：
- 竞争情报收集
- 多平台搜索结果比较
- AI 搜索引擎监控
...
创建一个你想要搜索的查询数组。在这个演示中，我们将查询不同城市的新闻：
...
<Tabs>
  ...
  <Tab title="ChatGPT">
    **解析器 ID:** `@olostep/chatgpt-results`
  ...
  <Tab title="Perplexity">
    **解析器 ID:** `@olostep/perplexity-results`
  ...
  <Tab title="Google AI Overview">
    **解析器 ID:** `@olostep/google-ai-overview-results`
  ...
  <Tab title="Microsoft Copilot">
    **解析器 ID:** `@olostep/microsoft-copilot-results`
...
**注意：** 提交后，[轮询完成状态](/use-cases/batch#step-2-monitor-batch-status)以检索结果。

## 响应格式

提交批量请求并轮询完成后，你将收到以下格式的响应：
...
下表概述了国家特定搜索的支持情况以及每个解析器消耗的积分。

| 功能               | Google AI Mode | ChatGPT | Perplexity | Google AI Overview | Gemini | Microsoft Copilot | Grok |
| ------------------- | :------------: | :-----: | :--------: | :----------------: | :----: | :---------------: | :--: |
| **支持国家**       |       ✅       |    ✅     |     ✅     |         ✅         |   ✅   |        ✅         |  ✅  |
...
缺少什么？请联系 `info@olostep.com` 以获取支持或定制实施协助。