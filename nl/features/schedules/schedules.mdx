...
- Geef elke gewenste payload door - de payload wordt precies verzonden zoals je het specificeert
...
Maak een schema om API-aanroepen automatisch uit te voeren. Je kunt eenmalige schema’s of terugkerende schema’s maken met behulp van cron-expressies. Het `endpoint` kan elke URL zijn (niet beperkt tot Olostep-eindpunten), en de `payload` kan elke data bevatten die je wilt verzenden.
...
Voor POST-verzoeken kun je korte vormen gebruiken voor Olostep-eindpunten. Het systeem zal automatisch `https://api.olostep.com/` voorvoegen:

- `v1/scrapes` - Plan webscraping-taken
- `v1/batches` - Plan batchverwerkingstaken
- `v1/crawls` - Plan website-crawlingoperaties
...
- Voor POST-verzoeken worden korte Olostep-eindpunten (`v1/scrapes`, `v1/batches`, `v1/crawls`, `v1/maps`, `v1/answers`) automatisch voorgeplaatst met `https://api.olostep.com/`
...
Schema’s zelf zijn gratis. Je betaalt alleen voor de API-aanroepen die worden uitgevoerd wanneer het schema wordt uitgevoerd. Bijvoorbeeld, als je een scrape plant, wordt er 1 credit per uitvoering in rekening gebracht (of meer als je parsers of LLM-extractie gebruikt).