...
title: "Dateien"
description: "Laden Sie JSON-Dateien hoch und verwalten Sie sie zur Verwendung als Kontext in API-Anfragen"
icon: "file-upload"
---

Über den Olostep `/v1/files` Endpunkt können Sie JSON-Dateien hochladen, die als Kontext in Ihren API-Anfragen verwendet werden können. Dies ermöglicht es Ihnen, strukturierte Daten bereitzustellen, um Ihre Scrapes, Antworten und andere Operationen zu verbessern.

- Laden Sie JSON-Dateien bis zu 200MB hoch
- Dateien werden automatisch auf korrektes JSON-Format validiert
- Verwenden Sie Dateien als Kontext in Scrapes, Antworten und anderen Endpunkten
...
```bash cURL
# macOS: builtin curl ist ausreichend
```
...
1. **Upload-URL erstellen**: Fordern Sie eine vorab signierte URL zum Hochladen Ihrer Datei an
2. **Upload abschließen**: Laden Sie Ihre Datei zur vorab signierten URL hoch und rufen Sie dann den Complete-Endpunkt auf, um zu validieren und abzuschließen

### Schritt 1: Upload-URL erstellen

Erstellen Sie zuerst eine Upload-URL, indem Sie den Dateinamen und den optionalen Zweck angeben. Der `purpose` Parameter unterstützt nur zwei Werte: `"context"` (Standard) oder `"batch"`.
...
Laden Sie Ihre JSON-Datei zur vorab signierten URL hoch und rufen Sie dann den Complete-Endpunkt auf, um den Upload zu validieren und abzuschließen.
...
# Nachdem Sie die upload_url aus Schritt 1 erhalten haben
file_id = upload_data["id"]
upload_url = upload_data["upload_url"]

# Bereiten Sie Ihre JSON-Daten vor
json_data = {
    "users": [
...
Der Complete-Endpunkt validiert die JSON-Datei und gibt Dateimetadaten zurück:
...
Rufen Sie Metadaten für eine Datei anhand ihrer ID ab.
...
Erhalten Sie eine vorab signierte URL, um den JSON-Inhalt einer abgeschlossenen Datei herunterzuladen. Optional können Sie die Ablaufzeit für die Download-URL mit dem `expires_in` Abfrageparameter angeben (Standard sind 600 Sekunden / 10 Minuten).

<CodeGroup>

```python Python
file_id = "file_abc123xyz789"
# Download-URL abrufen (Standardablauf: 600 Sekunden)
response = requests.get(
...
# Laden Sie den Dateiinhalte mit der vorab signierten URL herunter
...
# Download-URL mit benutzerdefiniertem Ablauf abrufen (3600 Sekunden = 1 Stunde)
curl -s -X GET "https://api.olostep.com/v1/files/$FILE_ID/content?expires_in=3600" \
  -H "Authorization: Bearer $OLOSTEP_API_KEY"

# Laden Sie die Datei mit der vorab signierten URL herunter
curl -s "$DOWNLOAD_URL"
```
...
Listen Sie alle abgeschlossenen Dateien für Ihr Team auf. Optional können Sie nach Zweck filtern (unterstützte Werte: `"context"` oder `"batch"`).
...
Löschen Sie eine Datei und ihre zugehörigen Daten aus dem Speicher.
...
Die hochgeladene Batch-Datei enthält eine gültige JSON-Struktur, die dem `/v1/batches` Endpunktformat entspricht:
...
- **Purpose-Parameter**: Unterstützt nur die Werte `"context"` oder `"batch"` (Standard ist `"context"`)

## Preisgestaltung

Das Hochladen von Dateien ist kostenlos. Dateien werden sicher gespeichert und verfallen automatisch nach 30 Tagen.