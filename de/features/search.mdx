...
- Erhalte strukturierte Links aus dem gesamten Web zurück

Es wird die Abfrage semantisch im gesamten Web durchsuchen und Ergebnisse zurückgeben.
...
Du erhältst ein `search`-Objekt als Antwort. Das `search`-Objekt enthält eine `id`, deine ursprüngliche `query` und ein `result` mit einer Liste von `links`.
