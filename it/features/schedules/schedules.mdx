...
description: "Pianifica le chiamate API per eseguirle automaticamente in orari specificati"
icon: "clock"
---

Attraverso l'endpoint Olostep `/v1/schedules` puoi pianificare le chiamate API per eseguirle automaticamente in orari specificati. Pianifica esecuzioni una tantum o attività ricorrenti utilizzando espressioni cron o linguaggio naturale.

- Pianifica esecuzioni una tantum in una data e ora specifica
...
Per i dettagli sull'API, consulta la [Riferimento API dell'Endpoint Programmazione](/api-reference/schedules/create).
...
Pianifica una chiamata API per eseguirla una volta in una data e ora specifica.
...
# Pianifica una raccolta dati da eseguire tra 1 ora
...
Crea una programmazione ricorrente usando un'espressione cron. Le espressioni cron utilizzano un formato a 6 campi: minuto ora giorno mese giorno-della-settimana anno.
...
# Pianifica una raccolta dati da eseguire ogni giorno alle 10:00 UTC
payload = {
    "method": "POST",
    "endpoint": "v1/scrapes",
    "payload": {
...
- `v1/maps` - Pianifica estrazione dati da mappe
...
- "ogni ora" → `0 * * * ? *`
- "ogni settimana di lunedì" → `0 0 ? * MON *`
...
- Il fuso orario deve essere un identificatore di fuso orario IANA valido (ad es., "UTC", "America/New_York", "Europe/London")
...
Le programmazioni stesse sono gratuite. Paghi solo per le chiamate API che vengono eseguite quando la programmazione viene eseguita. Ad esempio, se pianifichi una raccolta dati, ti verrà addebitato 1 credito per esecuzione (o più se utilizzi parser o estrazione LLM).