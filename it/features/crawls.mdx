...
sidebarTitle: "Crawl"
title: "Crawl"
description: "Effettua il crawl di un URL e ottieni il contenuto da tutte le sottopagine"
icon: "spider"
---

Attraverso l'endpoint Olostep `/v1/crawls` puoi effettuare il crawl di un sito web e ottenere il contenuto di tutte le pagine.

- Effettua il crawl di un sito web e ottieni il contenuto di tutte le sottopagine (o limita la profondità del crawl)
- Usa pattern speciali per effettuare il crawl di pagine specifiche (es. `/blog/**`)
...
Per i dettagli sull'API, consulta la [Riferimento API dell'Endpoint Crawl](/api-reference/crawls/create).
...
Interroga il crawl per tracciare i progressi fino a quando lo `status` è `completed`.
...
Recupera le pagine e iterale usando `cursor` e `limit`. Funziona mentre il crawl è `in_progress` o `completed`.
...
Il crawl costa 1 credito per pagina effettuata.