---
icon: "map"
sidebarTitle: "Risultati Google Maps"
title: "Ottieni Risultati Google Maps"
description: "API SERP per estrarre dati di localizzazione da Google Maps utilizzando Olostep e uno snippet di codice Python."
---

## Panoramica

L'API di Olostep ti consente di estrarre dati strutturati dalle localizzazioni di Google Maps utilizzando parser. Questi parser trasformano l'HTML grezzo delle pagine di Google Maps in dati JSON puliti e strutturati, pronti per l'analisi o l'integrazione nelle tue applicazioni.

## Esempio di Integrazione

Per ottenere contenuti JSON analizzati dai risultati di Google Maps, devi includere `json` nel parametro dei formati e specificare il nome del parser `@olostep/google-maps` nell'oggetto parser. Ecco come recuperare dati di localizzazione di Google Maps in un formato strutturato:
...
Quando fai una richiesta all'API di Olostep con il parser di Google Maps, riceverai una risposta JSON come l'esempio qui sotto:
...
- `Dettagli dell'attività` (nome, indirizzo, informazioni di contatto)
- `Informazioni su valutazioni e recensioni`
- `Orari di apertura`
- `Recensioni degli utenti con testo e immagini`
- `Luoghi simili nelle vicinanze`
- `Tag e servizi popolari`

## Risposta Strutturata: json_content

Il campo json_content contiene informazioni dettagliate sulla localizzazione. Ecco un esempio di dati che puoi aspettarti:
...
Olostep fornisce un file JSON ospitato con i dati di localizzazione strutturati. Puoi accedere al file JSON utilizzando il campo json_hosted_url nella risposta:

- JSON strutturato: [Visualizza esempio JSON](https://olostep-storage.s3.us-east-1.amazonaws.com/json_y6jwelcpdl.json)

Se vuoi ottenere anche il contenuto HTML e Markdown dei dati di localizzazione, puoi includere questi formati nel parametro dei formati e Olostep li restituirà nella risposta e fornirà URL ospitati per ciascun formato.

## Casi d'Uso

Il parser di Google Maps è particolarmente utile per:

Business Intelligence: Raccogli dati sulle localizzazioni dei concorrenti, valutazioni e feedback dei clienti
Ricerche di Mercato: Analizza recensioni dei clienti e sentimenti sulle attività in aree specifiche
Applicazioni Basate sulla Localizzazione: Integra informazioni dettagliate sulle attività e recensioni dei clienti
Analisi Immobiliare: Raccogli dati su servizi e attività locali nelle vicinanze
SEO Locale: Monitora la presenza delle attività e le recensioni dei clienti su Google Maps

## Note Importanti

Parametro Lingua: Il parametro ?hl=en imposta la lingua su Inglese. Regola secondo necessità per altre lingue.

## Conclusione

Utilizzare il parser di Google Maps con l'API di Olostep ti consente di estrarre dati di localizzazione strutturati che possono essere facilmente integrati nelle tue applicazioni o utilizzati per analisi. Il parser gestisce il compito complesso di estrarre informazioni dalle pagine di Google Maps, fornendoti dati puliti e strutturati. Per ottenere informazioni sui parser disponibili o per richiedere un parser personalizzato per il tuo caso d'uso specifico, contattaci all'indirizzo `info@olostep.com`