---
icon: "google"
sidebarTitle: "Risultati di Ricerca Google"
title: "Ottenere Risultati di Ricerca Google (python)"
description: "API SERP per estrarre risultati di ricerca Google utilizzando Olostep e uno snippet di codice Python."
---

## Panoramica

L'API di Olostep ti consente di estrarre dati strutturati dai risultati dei motori di ricerca utilizzando i [parser](../../features/structured-content/parsers). Questi parser trasformano l'HTML grezzo dei risultati di ricerca in dati JSON puliti e strutturati, pronti per l'analisi o l'integrazione nelle tue applicazioni.

## Esempio di Integrazione

Per ottenere contenuti JSON strutturati dai risultati di ricerca, devi includere `json` nel parametro `formats` e specificare il nome del parser `@olostep/google-search` nell'oggetto `parser`.

Ecco come recuperare i risultati di ricerca Google in un formato strutturato:
...
Quando fai una richiesta all'API di Olostep con il parser di Ricerca Google, riceverai una risposta JSON come l'esempio qui sotto:
...
`json_content` è la parte principale della risposta con i risultati di ricerca strutturati. Puoi accedere direttamente al contenuto JSON dalla risposta o utilizzare l'URL ospitato fornito nella risposta.
...
Se vuoi ottenere anche il contenuto HTML e Markdown dei risultati di ricerca, puoi includere questi formati nel parametro `formats` e Olostep li restituirà nella risposta fornendo URL ospitati per ciascun formato.
...
**Parametri di Ricerca**: I parametri `gl=us` e `hl=en` impostano la geolocalizzazione su US e la lingua su Inglese. Modificali secondo le tue necessità.

## Conclusione
Passare parser personalizzati all'API ti consente di ottenere solo i dati strutturati che desideri. Questo rende facile integrare i risultati di ricerca nelle tue applicazioni, eseguire analisi dei dati o costruire funzionalità correlate alla ricerca.

Per ottenere informazioni sui parser disponibili o per richiedere un parser personalizzato per il tuo caso d'uso specifico, contattaci all'indirizzo [info@olostep.com](mailto:info@olostep.com).