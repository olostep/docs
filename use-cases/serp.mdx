...
payload = {
    "formats": ["parser_extract", "markdown"],
...
When you make a request to the Olostep API with the Google Search parser, you'll receive a response like the example below:
...
This structured format makes it easy to integrate search results into your applications, perform data analysis, or build search-related features.



This structured format makes it easy to integrate search results into your applications, perform data analysis, or build search-related features.

For the URL in this example, https://www.google.com/search?q=alexander+the+great&gl=us&hl=en Olostep will also provide a hosted JSON file with the structured search results. You can access the JSON file using the following URL:
[View example JSON](https://olostep-storage.s3.us-east-1.amazonaws.com/json_94iqy385ty.json)

If you want to get also the html and markdown content of the search results, you can include the `html` and `markdown` formats in the `formats` parameter.
You will get also the html and markdown content of the search results in the response and two urls where we host the content for 7 days like these:
