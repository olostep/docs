...
<Tabs>
  <Tab title="Google AI Mode">
    **Parser ID:** `@olostep/google-aimode-scrape`
  ...
  <Tab title="ChatGPT">
    **Parser ID:** `@olostep/chatgpt-scrape`
  ...
  <Tab title="Perplexity">
    **Parser ID:** `@olostep/perplexity-scrape`
  ...
  <Tab title="Google AI Overview">
    **Parser ID:** `@olostep/google-ai-overview-scrape`
    ...
    const generateGoogleAIOverviewItems = () => {
      return CITIES.map((city, index) => {
        ...
        const encodedQuery = encodeURIComponent(query);
        return {
          url: `https://www.google.com/search?q=${encodedQuery}&gl=IN&hl=en`,
          custom_id: (index + 1).toString()
        };
      });
    };
    ...
    **Note:** The `gl` and `hl` parameters can be customized for different regions and languages.
  </Tab>

  <Tab title="Gemini">
    **Parser ID:** `@olostep/gemini-scrape`
  ...
  <Tab title="Microsoft Copilot">
    **Parser ID:** `@olostep/microsoft-copilot-scrape`
